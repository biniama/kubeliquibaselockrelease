...
public class LockDatabaseChangeLogGeneratorKubernetes extends LockDatabaseChangeLogGenerator {
    ...
    // LogFactory.getLog(LockDatabaseChangeLogGeneratorKubernetes.class);
    ...
    public Sql[] generateSql(LockDatabaseChangeLogStatement statement, Database database,
        ...
        updateStatement.addNewColumnValue("LOCKEDBY", String.format("%s:%s",
                KubernetesConnector.getInstance().getPodNamespace(), KubernetesConnector.getInstance().getPodName()));
        updateStatement.setWhereClause(database.escapeColumnName(liquibaseCatalog, liquibaseSchema,
                ...
                + " = 1 AND "
                + database.escapeColumnName(liquibaseCatalog, liquibaseSchema, database.getDatabaseChangeLogTableName(),
                        "LOCKED")
                + " = "
